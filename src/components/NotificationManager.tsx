...
export default function NotificationManager({ session }: { session: any }) {
  const { pushEnabled, permission, subscribe, checkStatus } = usePushNotifications(session);
  ...
  }, [session, permission, pushEnabled, checkStatus]);
  ...
  return (
    <>
      ...
      {activeToast && (
        <div className="fixed top-20 right-4 z-[9999] w-full max-w-sm animate-in slide-in-from-right-8 duration-300">
          <div className="bg-white/90 backdrop-blur-xl border-2 border-purple-500 rounded-3xl shadow-2xl p-6 ring-1 ring-black/5 overflow-hidden">
            ...
            <div className="flex gap-4">
              <div className="w-12 h-12 bg-purple-50 rounded-2xl flex items-center justify-center shrink-0 border border-purple-100">
                <Bell className="w-6 h-6 text-purple-600" />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="font-black font-outfit text-slate-900 truncate pr-4">{activeToast.title}</h4>
                <p className="text-sm font-medium text-slate-500 mt-1 line-clamp-2 leading-relaxed">{activeToast.body}</p>
                {activeToast.url && (
                  <Link 
                    href={activeToast.url} 
                    onClick={() => setActiveToast(null)}
                    className="inline-flex items-center gap-2 mt-4 text-xs font-black text-purple-600 uppercase tracking-widest hover:gap-3 transition-all"
                  >
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] border border-white/10 relative overflow-hidden">
             ...
             <div className="relative z-10 flex items-start gap-5">
               ...
               <div className="flex-1">
                 ...
                 <p className="text-white/60 text-sm font-medium mt-2 leading-relaxed">Enable push notifications to receive real-time travel alerts and spiritual updates.</p>
